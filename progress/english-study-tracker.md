...
- [ ] **反义疑问句 vs 一般疑问句**: 2026-02-16 全场满分，但 2026-02-17 出现 2 次回退（否定配否定、肯定配肯定）。尚未形成真正肌肉记忆。
...
- [ ] **词汇搭配地道性**: finally→in the end, beautiful→attractive, cases→case studies, by→with justify, how much→how large workforce, give impressions→impress people, sports→exercise。
...
12. [ ] **🔴 最高优先级: 漏译修饰词**: 翻译前系统扫描中文句子中的时间词(现在/早就/明天)、程度词(最/更/很/只/得多)、否定词(不/没)。2026-02-17 出现 **6 次**漏译，累计最严重的系统性问题。
13. [ ] **Tag question 再巩固**: 2026-02-17 出现 2 次回退（否定配否定、肯定配肯定），需恢复肌肉记忆。
14. [ ] **22BEC Start**: 进入 22BEC 新课文学习。
15. [ ] **Review Drill**: 间接疑问句语序（what the problem is）。
16. [ ] **Review Drill**: 抽查 hardly + tag question, subjunctive were。
17. [ ] **Review Drill**: 验证 `spend + doing` 和 phrasal verb 宾语规则。
18. [ ] **Review Drill**: 抽查 `what it's like` 嵌套结构 和 否定句 any + 复数。