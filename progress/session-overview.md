...
| 2026-05-30 | **15BEC 扫描 + 3 个新语言点训练 + Micro Role-play** | ✅ | 15BEC 扫描：21+ 个疑问点中仅 3 个在 overview 提及、其余全部未入池。提取 `get hold of`(6句式+QF全通关🏆) / `will do`(5句式+QF✅) / `be in`(6句式+QF, `Will you in`漏be关键性错误⚠️)。Flash Correct 情态动词+原形 3/3 零回退✅但Tag极性在情态动词干扰下复发。Micro RP 3 LP 融合成功。学生主动质疑导师 `get hold of` 定义遗漏(正确！)和 `will do` 反馈教条。🔴 `discuss` 及物性新缺口；`too more` 搭配。 |
| 2026-05-31 | **17BEC 扫描 + 4 个新语言点训练 + 话题构建训练** | ✅ | 17BEC 扫描：8 个疑问点中 4 个已记录(from 02/2026)，提取 `next week vs the next week`(🟢5句式+QF) / `do the schedules`(🟡5句式+QF, `do`搭配范围需巩固) / `delegator/delegate/delegatee`(🟡5句式+QF+加练, Tag时态一致新缺口) / `must go`(🟢5句式+QF)。Flash Correct 情态动词+原形 3/3 ✅ + 间接疑问句语序 2/2 ✅。Micro RP 3 LP 融合成功。话题构建-任务委托技巧(撤骨架满分🏆)。|
| 2026-06-04 | **18BEC 扫描 + 2 个新语言点训练 + Micro Role-play** | ✅ | 18BEC 扫描：8 个疑问点中 4 个已记录，2 个未练（`check vs confirm`/`interest`三句式）。提取 `with` 表"包含/附带"(🟡7句式+QF, `distraction`→`summary`严重词汇错误已修复) / `check + 省略 if/that`(🟡6句式+QF, 混合虚拟满分🏆)。Flash Correct 情态动词+原形 3 轮 + Tag 极性加练修复。Micro RP 2 LP 融合成功。🔴 Tag 极性再次复发；`ask for` 介词遗漏新缺口。AI 历史词组植入失职（0 次成功）。|
| 2026-06-06 | **19BEC 扫描 + 4 个新语言点 + 话题构建训练** | ✅ | 19BEC 扫描：6 个已记录 / 4 个未记录。LP 1 `stage/step in + doing`🟡 / LP 2 `need doing`🟡 / LP 3 `it is` 插入 wh- 从句🟡 / LP 4 `would` 假设语气🟢。Micro RP 2 LP 整合。话题构建(需求 Push Back) 4 步全走过。`get sb doing` 🔴→🟡 升级。|
| 2026-06-07 | **20BEC 扫描 + 4 个新语言点 + 话题构建（技术决策解释）** | ✅ | 20BEC 扫描：30+ 疑问点中 7 个已掌握，提取 `be about`(🟢) / `be of + noun`(🟢) / `or anything`(🟡) / `the sort of thing`(🟡)。Tag 极性同一 session 两次复发 ⚠️。Micro RP x2 整合。话题构建(技术决策与 trade-off 解释)4 步走过，撤骨架成功。🔴 `let sb do`(无to)+`allow sb to` 混淆新缺口。|
| 2026-06-15 | **22BEC 扫描 + 10 个新语言点训练 + Micro RP + Flash Correct x2** | ✅ | 22BEC 12 未记录 LP 中 10 个完成训练：`be worth doing`🟡 / `sit around`🟡 / `I'm assuming`🟡 / `check with+人+on+事`🟡 / `not in time for`🟡 / `next year's`🟡 / `go out`🟡 / `not...until`省略🟡 / `What's my budget?`🟡 / `as+adj`🟡。Flash Correct：过去分词 3/3✅ + `let/allow` 加练修复。🔴 完成时+过去时间 3 次复发；`catched`→`caught` 当场修复；`all hands on deck` 同 session 修复🏆。|
| 2026-06-14 | **21BEC 扫描 + 7 个新语言点 + 话题构建（技术决策解释）** | ✅ | 21BEC 扫描：12 组疑问点中 10 个未记录，提取 `good for`(🟡) / `up to date`(🟡) / `I can't see how...`(🟡) / `opinions on`(🟡) / `enough`(🟡) / `the sort of` 泛化(🟡) / `show + 宾 + 现分`(🟡) 共 7 个 LP。Flash Correct `discuss` 及物性 2复发后加练修复 + Tag 极性 2/2 ✅。Micro RP x2 整合。话题构建(技术决策与 trade-off)4 步全走过，撤骨架成功🏆。Tag 极性下午连续 5 次正确 🏆。新缺口：`more good`→`better` / `Would better` / `lack of` 混淆 / `very adj enough` / `distracting`→`distract` / `live`→`last`。`run into`🟢 成功复用 2 次。|
| 2026-06-16 | **23BEC 扫描 + 5 个新语言点训练 + Flash Correct** | ✅ | 23BEC 扫描：9 个未记录 LP。完成 5 个：`a choice of`🟡 / `choose N out of M`🟡 / `give sb the option to do`🟡 / `make notes/take notes`🟡 / `if you wish`🟡(仅1/6)。LP 4 `give a presentation on` 用户跳过。Flash Correct 完成时+过去时间 3/3✅但自由造句5次回退。`get onboard with`🔴→🟡 / `make lunch`🔴→🟡 修复。学生主动质疑 AI 植入不当×2。🔴 `not in time for` 后接类型；`familiar with` vs `used to` 混淆；Mixed 虚拟选择偏差。|
| 2026-06-20 | **23BEC 补齐 + 24BEC 扫描 + 8 LP + Flash Correct ×3 + 25BEC/27BEC 扫描 + 4 LP** | ✅ | 上午：23BEC 补齐 `if you wish`+24BEC 4 个 LP。下午：Flash Correct 间接疑问句语序 3/3 ✅。25BEC 扫描（12 疑问点, 3 已记录, 提取 `percentage graph`🟡+`if I can remember`🟡 均 5-6/6+QF）。27BEC 扫描（8 疑问点, 6 已记录, 提取 `priority` 可数/不可数🟡+`so to speak`🟡 均 4-5/6+QF+加练）。Micro RP（`percentage graph`+`if I can remember`）2 轮。26BEC-41BEC 速扫（28-41BEC 零未记录）。学生主动质疑 ×8（出题别扭×3/`take`不当提取×1/概念追问×2/植入失职×2）。🔴 Tag 时态一致性新次型；`so to speak` 位置错误；`might be made`→`might have been made`。AI 连续 4 轮植入失职（`so to speak` 全 LP）。24BEC 2 个 LP 未练已跳过。|
| 2026-06-21 | **42BEC 扫描 + 6 个新语言点 + Flash Correct ×2 + Micro RP ×2** | ✅ | 42BEC 扫描：学生 12 个疑问点中 5 个已训练、1 个部分覆盖、6 个未记录。提取 `stuff`(🟡5/6+QF, `and stuff like that` 即学即用🏆) / `change`不可数vs可数(🟡5/6+QF+1加练, R2关键性错误→加练修复) / `the same as`(🟡5/6+QF+2加练, 比较一致性R3+R5复发→修复) / `technical services`(🟡5/6+QF, 零关键性错误🏆) / `typically vs generally`(🟡5/6+QF, 区分正确) / `naturally vs in nature`(🟡5/6+QF, R4/R5满分🏆)。Flash Correct 情态+原形2/2+many/much2/2✅。Micro RP ×2 一次通关🏆。学生随堂概念辨析14次。🔴 新缺口：Comma Splice 10+次；造词习惯(untransparent/untrust/changability/batchly)；比较对象一致性；`there be`+`have`隔日复发；`about that`从句；词性混淆(副→名)；`pretty much`≠挺多；`I'm not agree`❌。`train up`/`trains colleagues` 学生跳过。|